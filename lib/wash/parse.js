      ...
      if (record.sigil == '@') {
        ...
        for (var i = 0; i < loose.length; i++) {
          ...
          if (el instanceof Array) {
            // If it's an array, we flatten it into the loose array.
            value.push.apply(value, el);
          } else {
            // Otherwise, push it as-is.
            value.push(el);
          }
        }

        args.setValue('_', value);
        ...
        for (var i = 0; i < loose.length; i++) {
          ...
          if (!(el instanceof Object))
            throw this.error('Expected object as loose argument');
          for (var key in el) {
            value[key] = el[key];
          }
        }
