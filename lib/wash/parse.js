  ...
  var nextArg = function() {
    ...
    if (this.lastToken == '-') {
      ...
      if (this.lastToken == '-') {
        ...
        this.advance(1);
        this.spanPattern(/[a-z0-9\-\_]*/ig);
        var name = this.lastToken;
        var record;
        if (/^no-/.test(name)) {
          name = name.substr(3);
        ...
        if (record.sigil == '$') {
          ...
          if (this.lastToken == '"' || this.lastToken == '\'') {
            record.setValue(this.parseString());
            return;
          }

          this.spanPattern(/\S/g);
          record.setValue(this.lastToken);
          return;
        }

        throw this.error('TODO: parse arg sigil: ' + record.sigil);
      }
    }
  };
 ...
 *
 * @param {string} expect A list of valid non-whitespace characters to terminate
 *   on, or an empty string to disable checking.
 * @return {string}
 */
