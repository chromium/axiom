// Copyright (c) 2015 Google Inc. All rights reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

import AxiomError from 'axiom/core/error';

import Arguments from 'axiom/fs/arguments';

/**
 * @constructor
 * @param {string} source
 * @param {number=} opt_pos
 */
export var Parse = function(source, opt_pos) {
  /** @type {string} */
  this.source = source;

  /** @type {number} */
  this.pos = opt_pos || 0;

  /** @type {string} */
  this.ch = this.source.substr(this.pos, 1);
};

export default Parse;

/**
 * @param {string} message
 * @return {AxiomError}
 */
Parse.prototype.error = function(message) {
  return new AxiomError.Parse(message, this.pos);
};

/**
 * Advance the current position
 * @param {number} count
 */
Parse.prototype.advance = function(count) {
  this.pos += count;
  this.ch = this.source.substr(this.pos, 1);
};

/**
 * @param {Object} signature
 * @param {Object=} opt_defaults
 * @return {Arguments}
 */
Parse.prototype.parseArgs = function(signature, opt_defaults) {
  var args = new Arguments(signature, opt_defaults || {});

  var loose = [];

  var nextArg = function() {
    if (this.ch == '-') {
      this.advance(1);
      if (this.ch == '-') {
        // double-dash parameter
        this.advance(1);
        var name = this.parsePattern(/[a-z0-9\-\_]+/ig);
        var record;
        if (/^no-/.test(name)) {
          name = name.substr(3);
          record = args.getRecord(name);
          if (!record)
            throw this.error('Unknown argument: ' + name);

          record.setValue(false);
          return;
        }

        record = args.getRecord(name);
        if (!record)
        throw this.error('Unknown argument: ' + name);

        if (record.sigil == '?') {
          record.setValue(true);
          return;
        }

        this.skipSpace();
        if (this.ch == '=') {
          this.advance(1);
          this.skipSpace();
        }

        if (record.sigil == '$') {
          record.setValue(this.parseSloppyString());
          return;
        }

        if (record.sigil == '%') {
          record.setValue(this.parseObject());
          return;
        }

        if (record.sigil == '@') {
          record.setValue(this.parseArray());
          return;
        }

        if (record.sigil == '*') {
          record.setValue(this.parseValue());
          return;
        }

        throw this.error('Unknown arg sigil: ' + record.sigil);
      }
    } else {
      // TODO: This is ok for when loose is an array, but not so great when
      // it's a string and the argument isn't quoted, since it'll eat the
      // intermediate whitespace.
      loose.push(this.parseSloppyString());
    }
  };

  this.skipSpace();

  while (this.pos < this.source.length) {
    var lastPos = this.pos;

    nextArg.call(this);

    if (this.pos == lastPos)
      throw this.error('internal error: parser did not advance position');

    this.skipSpace();
  }

  if (loose.length) {
    var record = args.getRecord('_');
    if (record) {
      if (record.sigil == '@') {
        args.setValue('_', loose);
      } else if (record.sigil == '$') {
        args.setValue('_', loose.join(''));
      } else {
        throw this.error('Can\'t parse loose sigil: ' + record.sigil);
      }
    }
  }

  return args;
};

/**
 * @param {RegExp} opt_pattern
 * @return {*}
 */
Parse.prototype.parseValue = function(opt_pattern) {
  if (this.ch == '[')
    return this.parseArray();

  if (this.ch == '{')
    return this.parseObject();

  return this.parseSloppyString(opt_pattern);
};

/**
 * @return {Array}
 */
Parse.prototype.parseArray = function() {
  if (this.ch != '[')
    throw this.error('Expected "["');

  this.advance(1);

  var rv = [];

  while (this.pos < this.source.length) {
    this.skipSpace();

    if (this.ch == ']') {
      this.advance(1);
      return rv;
    }

    if (this.ch == ',') {
      rv.push(null);
      this.advance(1);
      continue;
    }

    rv.push(this.parseSloppyString(/[^,\]]+/g));
    this.skipSpace();
    if (this.ch == ',') {
      this.advance(1);
      continue;
    }

    if (this.ch != ']')
      throw this.error('Expected "," or "]"');
  }

  throw this.error('Unterminated array');
};

/**
 * Parse a relaxed json object.
 *
 *   { foo: 1 }, { "foo": 1 }, { 'foo': 1 }
 *
 * @return {Object}
 */
Parse.prototype.parseObject = function() {
  if (this.ch != '{')
    throw this.error('Expected {');

  this.skipSpace();
  this.advance(1);

  // We'll allow sloppy strings that match this pattern as either a key
  // or a value.
  var sloppyPattern = /[^\s:,}]+/g;
  var rv = {};

  while (this.pos < this.source.length) {
    this.skipSpace();

    var name = this.parseSloppyString(sloppyPattern);
    this.skipSpace();
    if (this.ch != ':')
      throw this.error('Expected :');

    this.advance(1);
    this.skipSpace();

    var value = this.parseValue(sloppyPattern);
    rv[name] = value;

    this.skipSpace();
    if (this.ch == '}')
      return rv;

    if (this.ch != ',')
      throw this.error('Expected "," or "}"');

    this.advance(1);
  }

  throw this.error('Unterminated object literal');
};

/**
 * @param {parse.ParseState} parseState
 * @return {string}
 */
Parse.prototype.parseEscape = function() {
  if (this.ch == '"' || this.ch == '\'') {
    var rv = this.ch;
    this.advance(1);
    return rv;
  }

  throw this.error('TODO: parse escape: ' + this.ch);
};

/**
 * Parse a string that may or may not be quoted.
 * @param {RegExp=} opt_pattern
 * @return {string}
 */
Parse.prototype.parseSloppyString = function(opt_pattern) {
  if (this.ch == '"' || this.ch == '\'')
    return this.parseString();

  return this.parsePattern(opt_pattern || /\S+/g);
};

/**
 * Consume until the end of a string, handling escapes along the way.
 *
 * Escape sequences will be intact.
 *
 * @param {ParseState} parseState
 * @param {string} quote A single or double-quote character.
 * @return {string}
 */
Parse.prototype.parseString = function() {
  var result = '';

  var quote = this.ch;
  if (quote != '"' && quote != '\'')
    throw this.error('String expected');

  this.advance(1);

  var re = new RegExp('[\\\\' + quote + ']', 'g');

  while (this.pos < this.source.length) {
    re.lastIndex = this.pos;
    if (!re.exec(this.source))
      throw this.error('Unterminated string literal');

    result += this.source.substring(this.pos, re.lastIndex - 1);

    this.advance(re.lastIndex - this.pos - 1);

    if (quote == '"' && this.ch == '\\') {
      this.advance(1);
      result += this.parseEscape();
      continue;
    }

    if (quote == '\'' && this.ch == '\\') {
      result += this.ch;
      this.advance(1);
      continue;
    }

    if (this.ch == quote) {
      this.advance(1);
      return result;
    }
  }

  throw this.error('Unterminated string literal');
};

/**
 * Consume a particular pattern from the current parse position.
 *
 * @param {RegExp} pattern
 * @param {string=} opt_flags
 * @return {string}
 */
Parse.prototype.parsePattern = function(pattern, opt_flags) {
  if (!pattern.global)
    throw this.error('Span patterns must be global');

  pattern.lastIndex = this.pos;
  var ary = pattern.exec(this.source);

  if (!ary || pattern.lastIndex - ary[0].length != this.pos)
    throw this.error('Expected match for: ' + pattern);

  this.pos = pattern.lastIndex - 1;
  this.advance(1);

  return ary[0];
};

/**
 * Skip whitespace, return the next non-whitespace character.
 *
 * @param {string=} opt_expect A list of valid non-whitespace characters to
 *   terminate on.
 * @return {void}
 */
Parse.prototype.skipSpace = function(opt_expect) {
  if (!/\s/.test(this.ch))
    return;

  var re = /\s+/gm;
  re.lastIndex = this.pos;

  var source = this.source;
  if (re.exec(source))
    this.pos = re.lastIndex;

  this.ch = this.source.substr(this.pos, 1);

  if (opt_expect) {
    if (this.ch.indexOf(opt_expect) == -1) {
      throw this.error('Expected one of ' + opt_expect + ', found: ' +
          this.ch);
    }
  }
};
