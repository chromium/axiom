...
chromeAgentClient.openApiOnlineDoc = function(api) {
  var eventOrMethod = function(/** string */name) {
    return (name.substr(0, 2) === 'on' ? '#event-' : '#method-') + name;
  };
  ...
  if (api) {
    ...
    if (parts.length === 1)
      ...
      urlSuffix = parts[0] + eventOrMethod(parts[1]);
    else if (parts.length === 3)
      urlSuffix = parts[0] + '_' + parts[1] + eventOrMethod(parts[2]);
  }
