    ...
    function(error) {
      ...
      return this.findExecutable(pathSpec).then(
        function(/** {path: !Path, statResult: StatResult } */ result) {
          ...
          var buffer = new MemoryStreamBuffer();
          var stdio = new NestedStdio(this.executeContext.stdio);
          stdio.stdout = buffer.writableStream;
          ...
          if (true) {
            cxPromises.push(this.fileSystemManager.createOpenContext(
                new Path('html5:/log.txt'), 'wc'));
          }
          ...
          return Promise.all(cxPromises).then(
            function(cxs) {
              ...
              redirCx.open().then(function() {
                buffer.onData.addListener(function(data) {
                  console.log(data);
                  redirCx.write(0, SeekWhence.Current, DataType.Value, data)
                });
                buffer.resume();
              });
