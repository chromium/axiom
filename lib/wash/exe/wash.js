...
Wash.prototype.evaluate = function(str) {
  ...
  var command = this.parseShellInput(str);
  return this.dispatch(command).then(
    function(response) {
      this.setPrompt();
      return response;
    }.bind(this));
 ...
 * @constructor
 */
Wash.CommandLine = function(pathSpec, argv, redirIn, redirOut) {
  this.pathSpec = pathSpec;
  this.argv = argv;
...
Wash.prototype.parseShellInput = function(str) {
  ...
  return new Wash.CommandLine(path, argv, redirIn, redirOut);
};
 ...
 *
 * @param {Wash.CommandLine} redirPathSpec
 * @return {!Promise<*>}
 */
Wash.prototype.dispatch = function(cmd) {
  return this.findBuiltinOrExecutable(cmd.pathSpec).then(function(result) {
    var arg = this.parseArgv(result.statResult.signature, cmd.argv);
    var stdio = (cmd.redirIn || cmd.redirOut) ? 
        new NestedStdio(this.executeContext.stdio) : this.executeContext.stdio;

    return result.fileSystem.createExecuteContext(result.path, stdio, arg)
      .then(function(execCx) {
        if (cmd.redirIn) {
          var redirInBuffer = new ReadableFileStreamBuffer(
              execCx, this.absPath(cmd.redirIn.pathSpec), cmd.redirIn.mode);
          execCx.stdin = redirInBuffer.readableStream;
          execCx.stdin.onClose.addListener(function() {
            console.log('GOT IT');
            // stdio.close();
          });
        }
        ...
        if (cmd.redirOut) {
          var redirOutBuffer = new WritableFileStreamBuffer(
              execCx, this.absPath(cmd.redirOut.pathSpec), cmd.redirOut.mode);
          execCx.stdout = redirOutBuffer.writableStream;
        }
