...
Wash.prototype.parseShellCommand = function(str, pipeIn, pipeOut) {
  var interpolateFromHistoryWorked = false;

  var interpolateFromHistory = function(wash, match, cmdHint) {
    var cmd;
    if (!isNaN(+cmdHint)) {
      var cmdNum = wash.inputHistory.length - +cmdHint;
      if (cmdNum >= 0) {
        interpolateFromHistoryWorked = true;
        return wash.inputHistory[cmdNum];
      }
    } else {
      for (var i = 0; i < wash.inputHistory.length; ++i) {
        var item = wash.inputHistory[i];
        if (item.indexOf(cmdHint) === 0) {
          interpolateFromHistoryWorked = true;
          return item;
        }
      }
  ...
  str = str.replace(/!([-\w.]+)/g, interpolateFromHistory.bind(null, this));
  
  if (interpolateFromHistoryWorked) {
    // Print the interpollated command so the user knows what's being executed.
