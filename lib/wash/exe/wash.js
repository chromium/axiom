...
export var Wash = function(executeContext) {
  ...
  /** @type {JsFileSystem} */
  this.fileSystem = executeContext.fileSystem;
...
Wash.main = function(cx) {
  ...
  return function() {
    return wash.loadWashHistory().then(repl).catch(repl);
  };
};
    ...
    return this.fileSystem.stat(currentPath).then(
      function(statResult) {
        if (statResult.mode & Path.Mode.X)
          return Promise.resolve({absPath: currentPath, stat: statResult});
