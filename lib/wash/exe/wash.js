...
Wash.prototype.parseShellInput = function(str) {
  ...
  for (var i = 0; i < commandStrs.length; ++i) {
    ...
    // File redirection always takes precedence over piping (as on *nix).
    // TODO(ussuri): Given the above, if command N redirects stdin from or to
    // a file, commands N-1 and N+1 should adjust (use /dev/null to read/write).
    if (i > 0 && !cmd.redirIn) {
      cmd.redirPipeIn = {
        type: 'memory'
      };
 ...
 *
 * @param {Wash.Command} redirPathSpec
 * @return {!Promise<!Array<*>>}
 */
Wash.prototype.dispatch = function(commands) {
  ...
  //     this.dispatchOne.bind(this, commands[i], stdios[i])
  //   );
  // }
  // return promise;
};
...
Wash.prototype.dispatchOne = function(command, stdio) {
  return this.findBuiltinOrExecutable(command.pathSpec).then(function(result) {
    ...
    // TODO(ussuri): Could use this.executeContext.call() here.
...
Wash.prototype.dispatchExecuteContext = function(cx) {
  this.executeContext.addCallee(cx);
  return cx.execute();
};
