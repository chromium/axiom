...
export var Wash = function(executeContext) {
  ...
  for (var name in Wash.builtins) {
    builtins[name] = Wash.builtins[name][1].bind(null, this);
    builtins[name].signature = Wash.builtins[name][0];
  }
...
Wash.builtins = {
  'cd': [
    ...
    function(wash, cx) {
      ...
      /** @type {string} */
      var home = cx.getEnv('$HOME',
          cx.fileSystemManager.defaultFileSystem.rootPath.spec);
      /** @type {Path} */
      var path = wash.absPath(cx.arg || home);
