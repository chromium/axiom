...
export var Wash = function(executeContext) {
  ...
  // TODO(rpaquay): Get fsm from execution context?
  var fsm = new FileSystemManager();
  var jsfs = new JsFileSystem(fsm, 'jsfs-wash');
  fsm.mount(jsfs);
  this.builtinsFS = jsfs;
  this.builtinsFS.rootDirectory.install(builtins.callbacks);

  if (!this.executeContext.getEnv('$PWD'))
    this.executeContext.setEnv('$PWD', this.fileSystemManager.getFileSystems()[0].rootPath.spec);

  if (!this.executeContext.getEnv('$HOME'))
    this.executeContext.setEnv('$HOME', this.fileSystemManager.getFileSystems()[0].rootPath.combine('home').spec);
};
...
Wash.prototype.absPath = function(path) {
  return Path.abs(this.executeContext.getEnv('$PWD', this.fileSystemManager.getFileSystems()[0].rootPath.spec), path);
};
...
Wash.prototype.findExecutable = function(pathSpec) {
  // TODO(rpaquay): Is there a better default?
  var rootPath = this.fileSystemManager.getFileSystems()[0].rootPath;
...
Wash.prototype.findExeDirs = function(path) {
  return new Promise(function(resolve, reject) {
    var execs = [];
    this.fileSystemManager.list(new Path(path)).then(function(result) {
      var promises = [];
      var names = Object.keys(result);
      names.forEach(function(name) {
        promises.push(this.fileSystemManager.list(new Path(path + '/' + name)).then(function(r) {
          if (r && r['exe']) {
