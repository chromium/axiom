 ...
 *
 *    command [arg1 arg2 ...] [> some_file]
 *
 * @private
 * @type {RegExp}
 */
Wash.CMD_LINE_WITH_REDIRECT_RE_ =
    /^([^>]+?)(?:\s+([^>]+?))?(?:\s*(>>?)\s*(.+))?$/;
...
Wash.prototype.parseShellInput = function(str) {
  var match = Wash.CMD_LINE_WITH_REDIRECT_RE_.exec(str);
  var path = match[1];
  var argv = match[2] || null;
  var redirOp = match[3] || null;
  var redirPath = match[4] || null;
  ...
  return [path, argv, redirPath, redirOp === '>>' ? 'wc' : 'wct'];
};
