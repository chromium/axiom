...
Wash.builtins = {
  'cd': [
    ...
    function(wash, cx) {
      ...
      var pathSpec = list[0];
      if (!pathSpec) {
        pathSpec = cx.getEnv('$HOME', cx.getPwd());
      } else if (pathSpec === '-') {
        pathSpec = wash.prevCdDir;
      }
      var path = wash.absPath(pathSpec);
  ...
  'history': [
    ...
    function(wash, cx) {
      ...
      if (cx.getArg('clear', false)) {
        wash.inputHistory = [];
        cx.closeOk();
      } else {
        var list = cx.getArg('_', [wash.inputHistory.length]);
        if (list.length > 1 || isNaN(+list[0])) {
          cx.closeError(new AxiomError.TypeMismatch(
              'must be number of history items', list));
        }

        var itemCnt = +list[0];
        if (itemCnt > wash.inputHistory.length) {
          itemCnt = wash.inputHistory.length;
        }
        for (var i = itemCnt - 1; i >= 0; --i) {
          var printIdx = wash.inputHistory.length - i;
          cx.stdout.write(printIdx + ' ' + wash.inputHistory[i] + '\n');
        }
        cx.closeOk();
      }
    }
...
Wash.prototype.createStdios = function(commands) {
   ...
   * @param {!(MemoryStream|ReadableFileStreamBuffer|WritableFileStreamBuffer)} buffer
   * @return {void}
   */
  var closeBufferOnInterrupt = function(stdioResult, buffer) {
    stdioResult.onSignal.addListener(function(signal) {
  ...
  /**
   * @param {!CreateStdioResult} stdioResult
   * @param {!MemoryStream|!WritableFileStreamBuffer} buffer
   * @return {void}
   */
  var closeBufferOnStdioClose = function(stdioResult, buffer) {
   ...
   * @return {void}
   */
  var openRedirFileBuffer = function(wash, stdioResult, buffer, command) {
    closeBufferOnInterrupt(stdioResult, buffer);
  ...
  for (var i = 0; i < commands.length; ++i) {
    ...
    if (cmd.redirIn) {
      if (cmd.redirIn === Wash.RedirectInfo.PIPE) {
        ...
        var fileReader = new ReadableFileStreamBuffer(
            ...
            DataType.UTF8String,
            cmd.redirIn.mode);
        redirFilePromises.push(
            openRedirFileBuffer(this, stdioResult, fileReader, cmd));
    ...
    if (cmd.redirOut) {
      if (cmd.redirOut === Wash.RedirectInfo.PIPE) {
        curPipe = new MemoryStream();
        closeBufferOnInterrupt(stdioResult, curPipe);
        closeBufferOnStdioClose(stdioResult, curPipe);
        ...
        var fileWriter = new WritableFileStreamBuffer(
            ...
            cmd.redirOut.mode);
        redirFilePromises.push(
            openRedirFileBuffer(this, stdioResult, fileWriter, cmd));
        closeBufferOnStdioClose(stdioResult, fileWriter);
...
Wash.prototype.dispatchOne = function(command, stdioResult) {
  return this.findBuiltinOrExecutable(command.pathSpec).then(function(result) {
    ...
    var signalHandler = function(signal) {
      stdioResult.onSignal.fire(signal);
    };

    this.executeContext.stdio.signal.onData.addListener(signalHandler);

    // TODO(ussuri): Move this into ExecuteContext.call() and remove
    // opt_onClose there?
    var onClose = function(reason, value) {
      this.executeContext.stdio.signal.onData.removeListener(signalHandler);
