...
Wash.prototype.findExecutable = function(path) {
  ...
  var rootPath = new Path(this.fileSystemManager.getFileSystems()[0].name + ':');
  ...
  var searchNextPath = function() {
    ...
    var currentPath = rootPath.combine(currentPrefix + '/' + path);
...
Wash.prototype.dispatch = function(pathSpec, argv) {
  var path = new Path(this.builtinsFS.name + ':').combine(pathSpec);
