      ...
      .then(function(cx) {
        if (cmd.redirIn) {
          var redirInBuffer = new ReadableFileStreamBuffer(
              cx, this.absPath(cmd.redirIn.pathSpec), cmd.redirIn.mode);
          cx.stdin = redirInBuffer.readableStream;
          cx.stdin.onClose.addListener(function() {
          });
        }

        if (cmd.redirOut) {
          var redirOutBuffer = new WritableFileStreamBuffer(
              cx, this.absPath(cmd.redirOut.pathSpec), cmd.redirOut.mode);
          cx.stdout = redirOutBuffer.writableStream;
        }
