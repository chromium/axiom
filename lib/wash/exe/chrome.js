...
export var main = function(cx) {
  ...
  var tabs = cx.getArg('tabs', '').split(',');
  var pluck = cx.getArg('pluck', false);
  ...
  if (help ||
      ...
      (script && (argv.length !== 1 || tabs.length < 1)) ||
      (css && (argv.length !== 1 || tabs.length < 1)) ||
    ...
    cx.stdout.write([
      ...
      '    [-t|--timeout <ms>] [--all-frames] [--run-at start|end|idle]',
      ...
      '3) The --timeout value should be in milliseconds. The default is 1000ms.',
      '4) The --run-at option specifies at what point during document loading',
      '   the script is to be executed or CSS is to be inserted.'
    ].join('\r\n') + '\r\n');
    cx.closeOk();
    return;
  }
  ...
  var promise =
      ...
      script ? executeScript_(argv[0], tabs, options, pluck) :
      css ?    insertCss_(argv[0], tabs, options, pluck) :
               null;
