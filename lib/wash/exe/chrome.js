...
var CHROME_USAGE_STRING_ =
    ...
    '  chrome --execute-script|-s <code> [<tabId1>...]\n' +
    '  chrome --insert-css|-c <code> [<tabId1>...]';
...
export var main = function(cx) {
  ...
  var script = cx.getArg('run-script', false);
  ...
  if (installAgent) {
    ...
    var promise;
    if (api) {
      promise = chromeAgentClient.callApi(argv[0], argv.slice(1));
    } else if (script) {
      promise = chromeAgentClient.executeCodeInTabs(argv[0], argv.slice(1));
    } else if (css) {
      promise = chromeAgentClient.insertCssIntoTabs(argv[0], argv.slice(1));
    } else {
      cx.closeError(new AxiomError.Runtime('Unreachable'));
    }
