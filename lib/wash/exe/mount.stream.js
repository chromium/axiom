...
import ChannelFileSystem from 'axiom/fs/stream/file_system';
...
var mountStream_ = function(cx, fileSystemManager, name, path) {
  return fileSystemManager.readFile(path, DataType.UTF8String).then(
    function(/** ReadResult */ result) {
      ...
      var socket = new WebSocket(streamSrc);
      var streams = new WebSocketStreams(socket);
      var transport = new Transport(
          'websocket',
          streams.readableStream,
          streams.writableStream);
      var channel = new Channel('websocketchannel', transport);
      var fileSystem = new ChannelFileSystem(fileSystemManager, name, channel);
      fileSystem.description = 'streamfs - WebSocket - ' + streamSrc;
