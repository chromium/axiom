...
var createWash = function(jsfs, stdioSource) {
  // Note: this is a somewhat sneaky way of creating an instance of wash.
  // Ideally, we would invoke the wash executable instead of creating an
  // instance of it. In these tests however, we don't actually need to do that
  // as we currently only test the "execute" part of the wash "repl" loop.
...
describe('wash', function () {
  describe('redirection', function () {
    ...
    it('should allow reading from a file', function (done) {
      var testFullyCompleted = false;
      createJsfs().then(function(jsfs) {
        var path = new Path('jsfs:/test.txt');
        return jsfs.writeFile(path, DataType.UTF8String, 'hello there!')
          ...
          return createWash(jsfs, stdioSource).then(function(wash) {
            var path2 = new Path('jsfs:/test2.txt');
            var cmd = 'jsfs:/exe/sort <' + path.spec;
            return executeCommand(wash, cmd).then(function() {
              testFullyCompleted = true;
            });
          });
  ...
  it('should allow piping 2 commands', function (done) {
    var testFullyCompleted = false;
    createJsfs().then(function(jsfs) {
      var stdioSource = new StdioSource();
      return createWash(jsfs, stdioSource).then(function(wash) {
        var cmd = 'jsfs:/exe/echo foo | jsfs:/exe/sort';
        return executeCommand(wash, cmd).then(function() {
          testFullyCompleted = true;
        });
      });
  ...
  it('should allow piping 3 commands', function (done) {
    var testFullyCompleted = false;
    createJsfs().then(function(jsfs) {
      var stdioSource = new StdioSource();
      return createWash(jsfs, stdioSource).then(function(wash) {
        var cmd = 'jsfs:/exe/echo foo | jsfs:/exe/sort  | jsfs:/exe/sort -f';
        return executeCommand(wash, cmd).then(function() {
          testFullyCompleted = true;
        });
      });
