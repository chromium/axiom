...
export var main = function(cx) {
  ...
  /** @type {!string} */
  var singleFile = cx.getArg('file');
  singleFile = singleFile === undefined ? null : singleFile;

  if (list.length > 1 || cx.getArg('help')) {
    cx.stdout.write([
      'usage: import [<target directory>] [-f|--file <filename>]',
      'Import a directory from the local file system.',
      '',
      'If <target directory> is provided, the file(s) will be imported there.',
      'If not, it will be imported into the current directory.',
      '',
      'Imports a directory by default (if supported by browser).  If -f is'
      'provided, only a single file is imported (saved as <filename>).'
    ].join('\r\n') + '\r\n');
    cx.closeOk();
    return;
  }
  ...
  command.import(path, singleFile);
};
...
var ImportCommand = function(cx) {
  ...
  /** @private @type {!string} Filename to import if single file (or null if directory)*/
 ...
 * @param {!string} Filename to import if single file (or null if directory)
 * @return {void}
 */
ImportCommand.prototype.import = function(destination, singleFile) {
  this.destination = destination;
  this.singleFile_ = singleFile;
...
ImportCommand.prototype.handleFileSelect_ = function(evt) {
  ...
  var onFileLoad = function(data, evt) {
    ...
    if (this.singleFile_) {
      path = new Path(Path.join(path.spec, this.singleFile_));
    }
