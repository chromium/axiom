...
ImportCommand.prototype.handleDrop_ = function(event) {
  ...
  if (items) {
    ...
    Promise.all(traversalPromises).then(function(
        /** @type {!Array<File>} */ files) {
      this.importFileList_(files);
    })
  } else {
 ...
 */
ImportCommand.prototype.traverseFiles_ = function(file, path){
  if (file.isFile){
    ...
    dirReader.readEntries(function(entries) {
      ...
      for (var i = 0; i < entries.length; i++) {
        traversalPromises.push(this.traverseFiles_(entries[i],
            directoryPath + "/").then(function(traversedFiles) {
          files.concat(traversedFiles);
        }));
      }
      Promise.all(traversalPromises).then(function() {
        fileCompleter.resolve(files);
      })
    }.bind(this));
...
ImportCommand.prototype.handleFileLoadsCompletion_ = function(completions) {
  ...
  /** @type {Array<Error>} */
  var errors = completions.filter(function(element) { return element !== null; });
...
ImportCommand.prototype.destroy_ = function() {
  if (this.input_) document.body.removeChild(this.input_);
  if (this.dummyFocusInput_) document.body.removeChild(this.dummyFocusInput_);
