...
/**
 * Register a file system. The file system must have a unique name.
 *
 * @param {!FileSystem} fileSystem
 * @return {void}
 */
FileSystemManager.prototype.register = function(fileSystem) {
  // TODO(ussuri): The below is a consequence of FileSystemManager being a
  // FileSystem subclass itself. The FileSystem ctor calls register(), so we
  // end up here. Redo with a new minimal FileSystemBase.
  if (fileSystem == this)
    return;
 ...
 */
FileSystemManager.prototype.mount = function(fileSystemName) {
  var fileSystem = this.fileSystems_[fileSystemName];

  if (!fileSystem)
    throw new AxiomError.NotFound('unknown filesystem', fileSystemName);
  else if (fileSystem.getState() === Ephemeral.State.Ready)
    throw new AxiomError.Duplicate('filesystem already mounted', fileSystemName);

  return fileSystem.bringOnline().then(function() {
