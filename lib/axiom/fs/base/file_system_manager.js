...
/**
 * Mount a previously registered or a newly created file system.
 *
...
FileSystemManager.prototype.mount = function(fileSystem) {
  ...
  if (!fs)
    throw new AxiomError.NotFound('unknown filesystem', fsName);
  else if (fs.getState() === Ephemeral.State.Ready)
    throw new AxiomError.Duplicate('filesystem already mounted', fsName);
