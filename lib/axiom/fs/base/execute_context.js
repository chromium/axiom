 ...
 * @return {!Stdio}
 */
ExecuteContext.prototype.getStdio = function() {
  return new Stdio(
      this.stdin,
      this.stdout,
      this.stderr,
 ...
 * @return {!NestedStdio}
 */
ExecuteContext.prototype.createNestedStdio = function() {
  return new NestedStdio(this.getStdio());
};
...
ExecuteContext.prototype.call = function(fileSystemManager, path, arg) {
  return this.createExecuteContext(path, arg).then(
    function(cx) {
      this.setCallee(cx);
      return cx.execute();
    }.bind(this));
