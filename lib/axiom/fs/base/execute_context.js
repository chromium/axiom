...
export var ExecuteContext = function(fileSystem, stdio, path, args) {
  ...
  // Note: We merely declared the properties above (for closure compiler), we
  // now initialize them from stdio, including setting up event handlers if
  // needed.
  this.setStdio(stdio);
...
/**
 * Dispatch a signal to this execution context or callee if there is one.
 *
 * @param {!Signal} signal
 * @return {void}
 */
ExecuteContext.prototype.dispatchSignal = function(signal) {
  if (signal.name == 'tty-change' && signal.value instanceof TTYState) {
    this.setTTY(signal.value);
    return;
  }
