...
ExecuteContext.prototype.setStdio = function(stdio) {
  if (this.stdio)
    this.stdio.signal.onData.removeListener(this.dispatchSignal_);
 ...
 * @return {!Promise<*>}
 */
ExecuteContext.prototype.call = function(path, arg, opt_stdio, opt_onClose) {
  var stdio = new NestedStdio(opt_stdio || this.stdio, path.originalSpec);

  return this.createExecuteContext(path, stdio, arg).then(
    function(cx) {
