 ...
 * @param {*} arg
 */
export var ExecuteContext = function(fileSystem, stdio, path, arg) {
  Ephemeral.call(this);
...
ExecuteContext.prototype.setCallee = function(executeContext) {
  ...
  var oldStdio = this.callee.stdio;
  var newStdio = new Stdio(
    new ReadableStreamForwarder(oldStdio.stdin),
    oldStdio.stdout,
    oldStdio.stderr);
  this.callee.setStdio(newStdio);

  var onClose = function() {
    newStdio.stdin.close();
    this.callee.setStdio(oldStdio);
