...
/**
 * Installs an object of callback executables into the current directory.
 *
 * @param {*} An object with callback entries to be installed
 * @return {void}
 */
FileSystem.prototype.install = function(obj) {
  var result = this.rootDirectory.resolve(new Path('exe'));

  if (!result)
    throw new AxiomError.Invalid('path', null);
