...
JsFileSystem.prototype.alias = function(pathFrom, pathTo) {
  if (!pathFrom.isValid) {
    return Promise.reject(
      new AxiomError.Invalid('pathSpecFrom', pathFrom.originalSpec));
  }
  var resolveFrom = this.resolve(pathFrom);

  if (!pathTo.isValid) {
    return Promise.reject(
      new AxiomError.Invalid('pathSpecTo', pathTo.originalSpec));
  }
  ...
  if (resolveTo.isFinal) {
    ...
    if (!(resolveTo.entry instanceof JsDirectory)) {
      return Promise.reject(
        new AxiomError.Duplicate('pathSpecTo', pathTo.originalSpec));
    }
