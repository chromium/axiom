    ...
    if (resolveFrom.entry instanceof FileSystem) {
      ...
      if (resolveTo.entry == resolveFrom.entry) {
        return resolveFrom.entry.move(
            resolveFrom.entry.rootPath.combine(Path.join(resolveFrom.suffixList)),
            resolveFrom.entry.rootPath.combine(Path.join(resolveTo.suffixList)));
      }
  ...
  if (!rv.isFinal) {
    if (rv.entry instanceof FileSystem) {
      return rv.entry.createExecuteContext(
          rv.entry.rootPath.combine(Path.join(rv.suffixList)), arg);
    }
...
JsFileSystem.prototype.install = function(obj) {
  ...
  return this.list(new Path(rootName)).then(function (entries) {
    var exeDirName = 'exe';
    var exePath = new Path(rootName + exeDirName);
    if (!entries[exeDirName]) 
      return Promise.reject(new AxiomError.Missing('path ' + exePath.spec));
