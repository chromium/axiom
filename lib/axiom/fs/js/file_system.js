 ...
 *   to use as the root.
 */
export var JsFileSystem = function(fileSystemManager, name, opt_rootDirectory) {
...
JsFileSystem.prototype.stat = function(path) {
  ...
  if (rv.entry instanceof FileSystem)
    return rv.entry.stat(new Path(rv.entry.name).combine(Path.join(rv.suffixList)));
...
JsFileSystem.prototype.mkdir = function(path) {
  ...
  if (rv.entry instanceof FileSystem)
    return rv.entry.mkdir(new Path(rv.entry.name).combine(Path.join(rv.suffixList, targetName)));
    ...
    if (resolveFrom.entry instanceof FileSystem) {
      ...
      if (resolveTo.entry == resolveFrom.entry) {
        return resolveFrom.entry.move(
            new Path(resolveFrom.entry.name).combine(Path.join(resolveFrom.suffixList)),
            new Path(resolveFrom.entry.name).combine(Path.join(resolveTo.suffixList)));
      }
...
JsFileSystem.prototype.unlink = function(path) {
  ...
  if (rv.entry instanceof FileSystem) {
    return rv.entry.unlink(
       new Path(rv.entry.name).combine(Path.join(rv.suffixList, targetName)));
  }
...
JsFileSystem.prototype.list = function(path) {
  ...
  var rv = this.resolve(path);
  if (rv.entry instanceof FileSystem)
    return rv.entry.list(new Path(rv.entry.name).combine(Path.join(rv.suffixList)));
  ...
  if (!rv.isFinal) {
    if (rv.entry instanceof FileSystem) {
      return rv.entry.createExecuteContext(
        new Path(rv.entry.name).combine(Path.join(rv.suffixList)), arg);
    }
  ...
  if (!rv.isFinal) {
    if (rv.entry instanceof FileSystem) {
      return rv.entry.createOpenContext(
        new Path(rv.entry.name).combine(Path.join(rv.suffixList)), mode);
    }
