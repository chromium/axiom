...
JsFileSystem.prototype.install = function(obj) {
  return this.list(new Path('jsfs:')).then(function (entries) {
    if (!entries.exe) 
      return Promise.reject(new AxiomError.Missing('path', this.name + '/exe'));

    var result = this.rootDirectory.resolve(new Path('jsfs:exe/'));
    if (!result)
      return Promise.reject(new AxiomError.Missing('path', this.name + '/exe'));
