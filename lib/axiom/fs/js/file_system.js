 ...
 */
JsFileSystem.prototype.createExecuteContext = function(exePath, stdio, argDict) {
  ...
  if (!rv.isFinal) {
    if (rv.entry instanceof FileSystem) {
      return rv.entry.createExecuteContext(
          rv.entry.rootPath.combine(Path.join(rv.suffixList)), stdio, argDict);
    }
  ...
  if (rv.entry instanceof JsExecutable) {
    ...
    /** @type {!ExecuteContext} */
    var cx = new JsExecuteContext(this, stdio, exePath, rv.entry, args);
