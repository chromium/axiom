...
describe('JsFileSystem', function () {
  ...
  describe('when populated', function() {
    ...
    it('should have a method named "install" which returns <arg> + "baz"',
        function(done) {
          var callbacks = {
            'foo($)': function(cx) {
              cx.ready();
              cx.closeOk(cx.arg + 'baz');
            }
          }
          ...
          jsfs.rootDirectory.mkdir('exe').then(function (dir) {
            return jsfs.install(callbacks);
          }).then(function () {
            return jsfs.list(new Path('jsfs:exe/'))
          }).then(function (entries) {
            expect(entries['foo']).toBeDefined();
            return jsfs.createExecuteContext(new Path('jsfs:/exe/foo'), 'bar')
          }).then(function(cx) {
            return cx.execute()
