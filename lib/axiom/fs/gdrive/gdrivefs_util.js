  ...
  if (entry.downloadUrl) {
    ...
    if (opt_mimeType && entry.mimeType !== opt_mimeType) {
      return Promise.reject(new AxiomError.NotFound(
          'Mismatching MIME type', opt_mimeType));
    }
    ...
    if (!entry.exportLinks[opt_mimeType]) {
      return Promise.reject(new AxiomError.NotFound(
          'MIME type conversion unavailable', opt_mimeType));
    }
 ...
 */
gdrivefsUtil.createFile_ = function(parentDirId, name, mimeType) {
  return new Promise(function(resolve, reject) {
...
gdrivefsUtil.uploadFile = function(path, data, opt_mimeType) {
  return gdrivefsUtil.getEntry(new Path(path.getParentPath().relSpec))
    ...
    return gdrivefsUtil.createFile_(parentDir.id, path.getBaseName(), mimeType)
        .then(function(file) {
 ...
 * @return {Promise<Object>} Array of entries.
 */
