 ...
 *
 * @private
 * @param {!gapi.GDriveEntry} parentDirId The entry of the parent directory.
 * @param {!string} name The name of the file.
 * @param {string=} opt_mimeType The file's MIME type.
 * @return {!Promise} Operation's completion.
 */
gdrivefsUtil.createFile_ = function(parentDirId, name, opt_mimeType) {
  var mimeType = opt_mimeType || 'text/plain';
  return new Promise(function(resolve, reject) {
    var request = gapi.client.drive.files.insert({
        uploadType: 'media',
        parents: [{id: parentDirId}],
        title: name,
        mimeType: mimeType
    });
    request.execute(function(response) {
 ...
 * @return {!Promise<!Object<string, StatResult>>} Array of children's stats.
 */
 ...
 *
 * @private
 * @param {!string} entryId The ID of the entry to copy.
 * @param {!gapi.GDriveEntry} newParentDir The entry of the new parent dir.
 * @param {!string} newName The target's new name.
 * @return {!Promise} Operation's completion.
 */
gdrivefsUtil.copyEntry_ = function(entryId, newParentDir, newName) {
  return new Promise(function(resolve, reject) {
    var request = gapi.client.drive.files.copy({
        fileId: entryId,
        parents: [newParentDir],
        title: newName
    });
    request.execute(function(response) {
      if (!response.error) {
...
gdrivefsUtil.copyEntry = function(fromPath, toPath) {
  ...
  return gdrivefsUtil.getEntry(toPathPrefix).then(
    ...
    return gdrivefsUtil.getChildEntry_(toPrefixEntry, toPathSuffix).then(
      ...
      // source.
      return Promise.resolve(toSuffixEntry, fromPath.getBaseName());
    }).catch(function(error) {
      if (!AxiomError.NotFound.test(error)) {
        return Promise.reject(new AxiomError.Runtime(error));
      }
      // The prefix of the toPath was found, but the suffix was not: the
      // former is our targer dir, and the latter is our target name.
      return Promise.resolve(toPrefixEntry, toPathSuffix);
    });
  }).then(function(toDir, toName) {
    return gdrivefsUtil.getEntry(fromPath).then(function(fromEntry) {
      return gdrivefsUtil.copyEntry_(fromEntry, toDir, toName);
    });
  });
};
