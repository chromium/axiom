  ...
  return new Promise(function(resolve, reject) {
    if (window.gapi) {
      ...
      script.onerror = function() {
        reject(new AxiomError.Runtime('Google APIs not available'));
      }
  ...
  return new Promise(function(resolve, reject) {
    var onAuthorize = function(authResult) {
      if (authResult && authResult.access_token) {
        ...
        resolve(gdrivefsUtil.authenticateWithGDrive(false));
      } else {
        reject(new AxiomError.Runtime('Authentication failed'));
      }
    };
  ...
  return new Promise(function(resolve, reject) {
    ...
    request.execute(function(response) {
      if (!response.error) {
        ...
        reject(new AxiomError.Runtime(response.error.message));
      } else {
        reject(new AxiomError.Runtime(response.error.message));
      }
    });
...
gdrivefsUtil.getEntry = function(path) {
  ...
  for (var i = 0; i < path.elements.length; ++i) {
    ...
    promise = promise.then(function(curElt, curDir) {
      var mimeType =
          i < path.elements.length - 1 ? gdrivefsUtil.DIR_MIME_TYPE_ : null;
      return gdrivefsUtil.getChildEntry_(curDir.id, curElt, mimeType);
    }.bind(null, path.elements[i]));
  }
  return promise;
};
 ...
 *
 * @param {string} dirEntryId ID of the directory to search in.
 ...
 *
 * @param {Object} entry A GDrive entry to stat.
...
gdrivefsUtil.entryToStat_ = function(entry) {
  if (entry.mimeType === gdrivefsUtil.DIR_MIME_TYPE_) {
    ...
    return new StatResult({
      ...
      mode: mode,
      mtime: new Date(entry.modifiedTime).getTime(),
 ...
 *
 * @param {string} path Path to a GDrive entry to stat.
 ...
 *
 * @param {Object} entry The GDrive entry of the GDrive file to download.
  ...
  if (entry.downloadUrl) {
    ...
    if (opt_mimeType && entry.mimeType !== opt_mimeType) {
      return Promise.reject(new AxiomError.NotFound(
          'Mismatching MIME type', entry.title, opt_mimeType));
    }
    ...
    if (!entry.exportLinks[opt_mimeType]) {
      return Promise.reject(new AxiomError.NotFound(
          'MIME type conversion unavailable', entry.title, opt_mimeType));
    }
    return gdrivefsUtil.downloadUrl_(entry.exportLinks[opt_mimeType]);
  } else {
    // A Google doc unavailable for conversion (e.g. GMaps My Places).
    return Promise.reject(new AxiomError.NotFound(
        'Unavailable for download', entry.title));
  }
};
 ...
 *
 * @param {string} path The path of the GDrive file to download.
 * @param {string} opt_mimeType The desired MIME type:
 *       1) for "normal" files, must either be falsy or match the file's type;
 *       2) for GDrive docs, must match one of the conversion formats provided
 *          by GDrive for this file.
 ...
 *
 * @param {Object} parentDir The entry of the directory to put the file in.
 * @param {string} name The name of the file.
 * @param {string} mimeType The data's MIME type.
 * @return {Promise} Operation's completion.
 */
gdrivefsUtil.createFile_ = function(parentDir, name, mimeType) {
  return new Promise(function(resolve, reject) {
...
gdrivefsUtil.uploadFile = function(path, data, opt_mimeType) {
  return gdrivefsUtil.getEntry(new Path(path.getParentPath().relSpec))
      .then(function(parentDir) {
    var mimeType = opt_mimeType || 'text/plain';
    return gdrivefsUtil.createFile_(parentDir, path.getBaseName(), mimeType)
        .then(function(file) {
...
/**
 * Get a list of direct children for a given folderId.
 *
 * @param {string} entryId Entry ID of the directory.
 * @return {Promise<array<Object>>} Array of entries.
 */
 ...
 */
gdrivefsUtil.convertFileError = function(pathSpec, error) {
...
gdrivefsUtil.rejectFileError = function(pathSpec, reject, error) {
  reject(gdrivefsUtil.convertFileError(pathSpec, error));
};
