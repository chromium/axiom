...
export var WritableStreamForwarder = function(stream) {
  ...
  this.stream.onClose.listenOnce(function() {
    this.close();
  }.bind(this));
};
...
/**
 * Close the stream when there is no more data to stream (e.g. the underlying
 * file gets deleted or the file is remote and the system goes offline).
 *
 * @return {void}
 */
