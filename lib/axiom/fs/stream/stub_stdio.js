...
StubStdio.prototype.getStdio = function() {
  ...
  var stdin = this.readableStreams['stdin'];
  var stdout = this.writableStreams['stdout'];
  var stderr = this.writableStreams['stderr'];
  var signal = this.readableStreams['signal'];
  var ttyin = this.readableStreams['ttyin'];
  var ttyout = this.writableStreams['ttyout'];
  var result = new Stdio(stdin, stdout, stderr, signal, ttyin, ttyout);
  this.readableStreams.forEach(function(stream) {
    result[stream.name] = stream;
  });
  this.writableStreams.forEach(function(stream) {
    result[stream.name] = stream;
  });
  return result;
};
