 ...
 * @param {!ExecuteContext} execCx
 * @param {Path} filePath
 * @param {!OpenMode} fileMode
 */
export var WritableFileStreamBuffer = function(fileSystem, filePath, fileMode) {
  StreamBuffer.call(this);

  /** @const @type {!WritableStream} */
  this.writableStream = new WritableMemoryStream(this);

  // TODO(ussuri): Also might need to stop write() calls below in midflight.
  this.onClose = this.writableStream.onClose;

  fileSystem.createOpenContext(filePath, fileMode)
    .then(function(writeCx) {
      writeCx.open().then(function() {
        this.onData.addListener(function(data) {
          writeCx.write(0, SeekWhence.End, DataType.UTF8String, data);
        });
        this.resume();
      }.bind(this));
    }.bind(this));
 ...
 * @return {void}
 */
WritableFileStreamBuffer.prototype.close = function() {
  // Also will indirectly fire onClose. 
