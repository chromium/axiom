...
export var ChromePortStreams = function() {
  ...
  // Bind event handlers
  this.onConnectingMessageBound_ = this.onConnectingMessage_.bind(this);
  this.onConnectingDisconnectBound_ = this.onConnectingDisconnect_.bind(this);
  this.onPortMessageBound_ = this.onPortMessage_.bind(this);
  this.onPortDisconnectBound_ = this.onPortDisconnect_.bind(this);
};
...
ChromePortStreams.prototype.setPort = function(port, alreadyConnected) {
  ...
  if (alreadyConnected) {
    ...
    this.connectionState_ = BufferedStreamsSource.ConnectionState.CONNECTING;
    port.onMessage.addListener(this.onConnectingMessageBound_);
    port.onDisconnect.addListener(this.onConnectingDisconnectBound_);
...
ChromePortStreams.prototype.onPortDisconnect_ = function(event) {
  this.connectionState_ = BufferedStreamsSource.ConnectionState.CLOSED;
  this.port_.onMessage.removeListener(this.onPortMessageBound_);
  this.port_.onDisconnect.removeListener(this.onPortDisconnectBound_);
  this.port_ = null;
...
ChromePortStreams.prototype.handleConnected_ = function() {
  ...
  this.port_.onMessage.addListener(this.onPortMessageBound_);
  this.port_.onDisconnect.addListener(this.onPortDisconnectBound_);
  this.onConnect.fire();
};
...
ChromePortStreams.prototype.cleanupConnectionListeners_ = function() {
  this.port_.onMessage.removeListener(this.onConnectingMessageBound_);
  this.port_.onDisconnect.removeListener(this.onConnectingDisconnectBound_);
};
