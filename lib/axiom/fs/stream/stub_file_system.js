...
StubFileSystem.prototype.createExecuteContext = function(path, stdio, arg) {
  ...
  this.createStubStdio_(stdio).then(
    function(stubStdio) {
      // Open context on remote end.
      return this.sendRequest_({
...
StubFileSystem.prototype.createStubWritableStream_ =
  ...
  return this.sendRequest_({cmd: 'writable-stream.create'}).then(
    function(streamId) {
      var stream = new StubWritableStream(this, stream, streamName, streamId);
      stubStdio.writableStreams[streamName] = stream;
      stream.dependsOn(stubStdio);
      return Promise.resolve();
    }.bind(this)
  );
};
...
StubFileSystem.prototype.createStubReableStream_ =
  ...
  return this.sendRequest_({cmd: 'readable-stream.create'}).then(
    function(streamId) {
      var stream = new StubReadableStream(this, stream, streamName, streamId);
      stubStdio.readableStreams[streamName] = stream;
      stream.dependsOn(stubStdio);
      return Promise.resolve();
    }.bind(this)
  );
