...
/** @typedef BufferedStreamsSource$$module$axiom$fs$stream$buffered_streams_source */
var BufferedStreamsSource;
...
BufferedStreams.prototype.flushWriteQueue_ = function() {
  while(this.getState_() === BufferedStreams.ConnectionState.CONNECTED) {
    ...
    if (item instanceof EventWithCallback) {
      this.source.write(item.value, item.callback);
    } else {
      this.source.write(item);
    }
  }
};
...
BufferedStreams.prototype.onSourceConnect_ = function() {
  if (this.source_.getStat() != BufferedStreams.ConnectionState.CONNECTED)
...
BufferedStreams.prototype.onSourceClose_ = function(error) {
  if (this.source_.getStat() != BufferedStreams.ConnectionState.CLOSED)
