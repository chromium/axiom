 ...
 */
export var StreamBuffer = function(opt_name) {
  /** @type {string} */
...
/**
 * Can be overwritten by subclasses in case closing is non-trivial.
 * 
 * @return {void}
 */
StreamBuffer.prototype.close = function() {
  this.assertOpen();
  this.closed_ = true;
  if (!this.paused_) {
    this.flushEvents_();
 ...
 * @return {void}
 */
StreamBuffer.prototype.assertOpen = function() {
  if (this.closed_)
    throw new AxiomError.Runtime('Buffer ' + this.name + ' is already closed');
};
