 ...
 * @param {?} fileSystem The local node file system
 */
export var NodeFileSystem = function(fileSystemManager, name, fileSystem) {
 ...
 * @param {?} fs a node filesystem object.
 */
NodeFileSystem.mount = function(fileSystemManager, fileSystemName, fs) {
  var nodefs = new NodeFileSystem(fileSystemManager, fileSystemName, fs);
  fileSystemManager.mount(nodefs);
  return nodefs;
};
...
NodeFileSystem.prototype.resolve = function(path) {
  ...
  return new Promise(function(resolve, reject) {
    nodefs.root.getFile(NodeFileSystem.makeNodefsPath_(path), {create: true}, resolve, reject);
  });
};
...
NodeFileSystem.prototype.stat = function(path) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    this.fileSystem.stat(NodeFileSystem.makeNodefsPath_(path), statCb);
  }.bind(this));
};
...
NodeFileSystem.prototype.mkdir = function(path) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    this.fileSystem.mkdir(NodeFileSystem.makeNodefsPath_(path), cb);
...
NodeFileSystem.prototype.unlink = function(path) {
  ...
  return new Promise(function(resolve, reject) {
    this.fileSystem.unlink(NodeFileSystem.makeNodefsPath_(path), function(err) {
      if (err) {
        return reject(err);
      } else {
        return resolve(null);
...
NodeFileSystem.prototype.list = function(path) {
  ...
  return nodefsUtil.listDirectory(this.fileSystem, NodeFileSystem.makeNodefsPath_(path)).then(
    function(entries) {
