...
/**
 * Mounts a given type of node filesystem at /jsDir/mountName
 *
 * @param {!FileSystemManager} fileSystemManager
...
NodeFileSystem.prototype.stat = function(path) {
  ...
  return nodefsUtil.statPath(this.nodefs, path);
};
 ...
 */
NodeFileSystem.prototype.createExecuteContext = function(path, stdio, argDict) {
  if (!path.isValid)
    return Promise.reject(new AxiomError.Invalid('path', path.originalSpec));

  /** @type {!ExecuteContext} */
  var cx = new NodeExecuteContext(this, stdio, path, argDict);
