 ...
 * @return {void}
 */
DomFileSystem.prototype.rejectInvalidPath_ = function(path) {
  if (!path || !path.isValid || path.root != this.name)
...
DomFileSystem.prototype.resolve = function(path) {
  this.rejectInvalidPath_(path);

  //TODO(grv): resolve directories and read mode bits.
  var domfs = this.fileSystem;
  return new Promise(function(resolve, reject) {
    domfs.root.getFile(path.relSpec, {create: true}, resolve, reject);
  });
};
...
DomFileSystem.prototype.stat = function(path) {
  ...
  return domfsUtil.getFileOrDirectory(this.fileSystem.root, path.relSpec).then(
      function(r) {
...
DomFileSystem.prototype.mkdir = function(path) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var parentPathSpec = parentPath.relSpec || '/';
...
DomFileSystem.prototype.list = function(path) {
  ...
  return domfsUtil.listDirectory(this.fileSystem.root, path.relpec).then(
    function(entries) {
