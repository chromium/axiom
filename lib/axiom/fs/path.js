...
var Path = function(spec) {
  ...
  /** @type {string} */
  this.relSpec = this.elements.join(Path.separator);

  /** @type {string} */
  this.spec = this.root + Path.rootSeparator + Path.separator + this.relSpec;
};
...
Path.abs = function(pwd, pathSpec) {
  ...
  if (pathSpec[0] == '/') {
    ...
    var fileSystemName = pwd.split(':')[0];
    return new Path(fileSystemName + ':' + pathSpec);
  }
