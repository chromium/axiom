...
/**
 * Create an event that fires whenever any of the events passed as arguments
 * fires. When opt_fireOnce is true, the event fires only once, otherwise the
 * event fires every time.
 *
 * @param {!Array<!AxiomEvent>} events
 * @param {boolean=} opt_fireOnce
 * @return {!AxiomEvent}
 */
AxiomEvent.any = function(events, opt_fireOnce) {
  var fireOnce = opt_fireOnce || false;
  var event = new AxiomEvent();
  ...
  for (var i = 0; i < events.length; ++i) {
    if (fireOnce) {
      events[i].listenOnce(listener);
    } else {
      events[i].addListener(listener);
    }
  }
...
/**
 * Create an event that fires as soon as all of the events passed as arguments
 * have fired at least once. When opt_fireOnce is true, the event fires only
 * once, otherwise the process resets after each firing.
 * 
 * @param {!Array<!AxiomEvent>} events
 * @param {boolean=} opt_fireOnce
 * @return {!AxiomEvent}
 */
AxiomEvent.all = function(events, opt_fireOnce) {
  var fireOnce = opt_fireOnce || false;
  var event = new AxiomEvent();
  var firedEvents = [];
  var listeners = [];

  var listener = function(eventIdx, var_args) {
    firedEvents[eventIdx] = true;
    if (firedEvents.every(function(elt) { return elt === true; })) {
  ...
  for (var i = 0; i < events.length; ++i) {
    firedEvents[i] = false;
    listeners[i] = listener.bind(null, i);
    if (fireOnce) {
      events[i].listenOnce(listeners[i]);
    } else {
