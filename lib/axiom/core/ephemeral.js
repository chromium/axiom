...
/**
 * @param {Ephemeral} otherEphemeral
 */
Ephemeral.prototype.dependsOn = function(otherEphemeral) {
  var onCloseHandler = function(reason, value) {
    if (this.isEphemeral('Closed', 'Error'))
      return;

    this.closeError(new AxiomError.ParentClosed(reason, value));
  }.bind(this);

  otherEphemeral.onClose.addListener(onCloseHandler);

  this.onClose.addListener(function(reason, value) {
    otherEphemeral.onClose.removeListener(onCloseHandler);
  });
};
